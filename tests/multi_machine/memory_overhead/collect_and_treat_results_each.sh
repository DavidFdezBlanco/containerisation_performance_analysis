...
echo -e "number of cluster,machine,index,Total time,Total operations,Operations per second,MiB/sec" >> "$output_file"

for ((i = 1; i <= number_executions; i++))
do
    filename="/tmp/perf_study/test/$engine/results/untreated_${engine}_${node}_memory_overhead_$i.txt"
    ...
    echo -e "$number_cluster,$node,$i,$total_time,$total_operations,$operations_per_second,$mib_per_sec" >> "$output_file"
    rm "$filename"
done