...
for i in $(seq 1 $repetitions); do
    # Create new containers based on the "memory_lxd" image
    echo "Launching image_lxd_memory_overhead Image..."
    sudo lxc launch image_lxd_memory_overhead lxdmemoryOverhead$i --target $node
    ...
    sudo lxc exec lxdmemoryOverhead$i -- sysbench memory run > /tmp/perf_study/test/lxd/results/untreated_lxd_${node}_memory_overhead_$i.txt
