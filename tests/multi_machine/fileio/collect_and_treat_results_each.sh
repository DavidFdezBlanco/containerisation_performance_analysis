...
echo -e "number of cluster,machine,test,index,Throughput read MiB/s,Throughput written MiB/s" >> "$output_file"

for ((i = 1; i <= number_executions; i++))
do
    filename="/tmp/perf_study/test/$engine/results/untreated_${engine}_${node}_fileio_${filetestmode}_$i.txt"
    ...
    echo -e "$number_cluster,$node,$filetestmode,$i,$read_throughput,$written_throughput" >> "$output_file"
    rm "$filename"
done