...
for i in $(seq 1 $repetitions); do
    ...
    sudo lxc launch image_lxd_mutex lxdMutex$number$i --target $node
    ...
    sudo lxc exec lxdMutex$number$i -- sysbench mutex --threads=$number run > /tmp/perf_study/test/lxd/results/untreated_lxd_${node}_mutex_${number}_$i.txt
