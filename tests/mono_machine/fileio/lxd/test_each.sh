...
for i in $(seq 1 $repetitions); do
    ...
    sudo lxc launch image_lxd_fileio lxdFileio$filetestmode$i
    ...
    sudo lxc exec lxdFileio$filetestmode$i -- sysbench fileio --file-test-mode=$filetestmode run > /tmp/perf_study/test/lxd/results/untreated_lxd_fileio_${filetestmode}_$i.txt
