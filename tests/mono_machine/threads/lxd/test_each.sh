...
for i in $(seq 1 $repetitions); do
    ...
    sudo lxc launch image_lxd_threads lxdThreads$number$i
    ...
    sudo lxc exec lxdThreads$number$i -- sysbench threads --threads=$number run > /tmp/perf_study/test/lxd/results/untreated_lxd_threads_${number}_$i.txt
