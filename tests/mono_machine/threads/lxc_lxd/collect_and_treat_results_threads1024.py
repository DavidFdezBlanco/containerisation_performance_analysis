...
output_file = "resultats_Threads1024.xlsx"

# Création du fichier Excel avec les titres de colonnes
wb = openpyxl.Workbook()
ws = wb.active
ws.append(["Fichier", "Total Time",  "Total Number of Events", "Events (avg)", "Events (stddev)", "Execution Time (avg)", "Execution Time (stddev)"])
...
for filename in file_names:
    ...
    with open(os.path.join(result_dir, filename), "r") as f:
        ...
        for line in lines:
            ...
            if total_time and total_events and events_avg and events_stddev and execution_time_avg and execution_time_stddev:
                break

        # Écriture des données dans le fichier Excel
        ws.append([filename, total_time, total_events, events_avg, events_stddev, execution_time_avg, execution_time_stddev])
