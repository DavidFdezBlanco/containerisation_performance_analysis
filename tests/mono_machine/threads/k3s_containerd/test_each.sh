...
for i in $(seq 1 $repetitions); do
  ...
  kubectl exec -it "$container_name" -- sysbench threads --threads=$number run > /tmp/perf_study/test/k3s_containerd/results/untreated_k3s_containerd_threads_${number}_$i.txt
